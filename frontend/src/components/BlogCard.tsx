...
export const BlogCard =({
    ...
    <div className="p-4 border-b border-slate-200 pb-4 w-screen max-w-screen-md cursor-pointer">
        <div className="flex">
            ...
            <div className="font-extralight pl-2 capitalize text-sm flex justify-center flex-col">
            {authorName}
            </div>
...
export function Avatar({name,size="small"}:{name:string, size?:"small" | "large"}){
    ...
    <div onClick={()=>{
        ...
        <span className={`${size==="small"?"text-xs":"text-md"} text-gray-600 dark:text-gray-300 uppercase `}>{name.charAt(0)}</span>
    </div>
    )
